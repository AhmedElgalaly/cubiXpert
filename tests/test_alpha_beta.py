...
class AlphaBetaTester:
    ...
    def get_move(self, game):
        ...
        move, _ = self._alphabeta(game, self.depth, -math.inf, math.inf, True)
    ...
    def _alphabeta(self, game, depth, alpha, beta, maximizing):
        ...
        if maximizing:
            ...
            for move in moves:
                ...
                _, val = self._alphabeta(game, depth-1, alpha, beta, False)
            ...
            for move in moves:
                ...
                _, val = self._alphabeta(game, depth-1, alpha, beta, True)
                game.undo_move(x, y, z)
...
def run_alpha_beta_tests(num_games=10):
    tester = AlphaBetaTester(depth=2)
    log = []
    ...
    for i in range(num_games):
        ...
        while True:
            ...
            if win:
                result = "AI Wins" if current_player == 0 else "AlphaBeta Wins"
    ...
    # Write log file
    with open("tests/alphabeta_results.txt", "w") as f:
        f.write("\n".join(log))
        f.write(f"\n\nFinal Results ({num_games} games):\n")
        f.write(f"AlphaBeta Wins: {tester.total_wins}\n")
